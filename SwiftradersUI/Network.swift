...
class Network:ObservableObject {
    ...
    @Published var shipList : ShipList = ShipList()
    ...
    func listShips() {
        guard let url = URL(string: "https://api.spacetraders.io/v2/my/ships") else { fatalError("Missing URL") }
        ...
        let dataTask = URLSession.shared.dataTask(with: urlRequest) { (data, response, error) in
            ...
            if response.statusCode == 200 {
                ...
                DispatchQueue.main.async {
                    do {
                        print(String(data: data, encoding: String.Encoding.utf8) as Any)
                        let decodedShipList = try JSONDecoder().decode(ShipList.self, from: data)
                        self.shipList = decodedShipList
                    } catch let error {
                        print("Error decoding: ", error)
                    }
                }
